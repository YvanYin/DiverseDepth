...
if __name__ == '__main__':
    ...
    imgs = sio.loadmat(base_dir + '/NYUDepthV2/test/test.mat')
    rgbs = imgs['rgbs']
    depths = imgs['depths']
    ...
    for i in range(test_datasize):
        ...
        cv2.imwrite(os.path.join(image_dir, img_name[-4:] + '_rgb.jpg'), np.squeeze(rgb))
        plt.imsave(os.path.join(image_dir, img_name[-4:] + '_gt.png'), np.squeeze(depth), cmap='rainbow')
        cv2.imwrite(os.path.join(image_dir, img_name[-4:] + '_gtraw.png'), (pred_depth_metric * 6000).astype(np.uint16))
